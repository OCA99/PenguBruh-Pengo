...
void Pengo::construct() {
	...
	rect.x = 32;
	Animation* pushLeft = new Animation(sprites, &rect, 1, 2, 10, 1, [&] { pushing = false; animator->setCurrentState(PengoAnimations::WalkLeft); });
	(*states)[PengoAnimations::PushLeft] = pushLeft;
	rect.x = 64;
	Animation* pushUp = new Animation(sprites, &rect, 1, 2, 10, 1, [&] { pushing = false; animator->setCurrentState(PengoAnimations::WalkUp); });
	(*states)[PengoAnimations::PushUp] = pushUp;
	rect.x = 96;
	Animation* pushRight = new Animation(sprites, &rect, 1, 2, 10, 1, [&] { pushing = false; animator->setCurrentState(PengoAnimations::WalkRight); });
