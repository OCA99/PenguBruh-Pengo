...
Update_Status ModulePlayer::Update()
{
	...
	if (!moving && App->input->keys[SDL_SCANCODE_SPACE] == Key_State::KEY_DOWN) {
		switch (direction) {
		case Directions::Up:
			if (!App->blocks->BlockInGridPosition(gridPosition.x, gridPosition.y - 1)) App->walls->PushWall(0);
			App->blocks->PushBlock(gridPosition.x, gridPosition.y, gridPosition.x, gridPosition.y - 1);
			pushUpAnim.Reset();
			currentAnimation = &pushUpAnim;
			break;
		case Directions::Down:
			if (!App->blocks->BlockInGridPosition(gridPosition.x, gridPosition.y + 1)) App->walls->PushWall(1);
			App->blocks->PushBlock(gridPosition.x, gridPosition.y, gridPosition.x, gridPosition.y + 1);
			pushDownAnim.Reset();
			currentAnimation = &pushDownAnim;
			break;
		case Directions::Left:
			if (!App->blocks->BlockInGridPosition(gridPosition.x - 1, gridPosition.y)) App->walls->PushWall(2);
			App->blocks->PushBlock(gridPosition.x, gridPosition.y, gridPosition.x - 1, gridPosition.y);
			pushLeftAnim.Reset();
			currentAnimation = &pushLeftAnim;
			break;
		case Directions::Right:
			if (!App->blocks->BlockInGridPosition(gridPosition.x + 1, gridPosition.y)) App->walls->PushWall(3);
			App->blocks->PushBlock(gridPosition.x, gridPosition.y, gridPosition.x + 1, gridPosition.y);
			pushRightAnim.Reset();
